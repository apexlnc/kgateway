	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(aiIr *AIIr) bool {
				...
				return provider.Routes["/v1/chat/completions"] == api.AIBackend_ROUTE_TYPE_COMPLETIONS &&
					provider.Routes["/v1/messages"] == api.AIBackend_ROUTE_TYPE_MESSAGES &&
					provider.Routes["/v1/models"] == api.AIBackend_ROUTE_TYPE_MODELS &&
					provider.Routes["*"] == api.AIBackend_ROUTE_TYPE_PASSTHROUGH
			},
		},
	}
	for _, tt := range tests {
